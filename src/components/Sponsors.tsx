  ...
  return (
    <section id="sponsors" className="py-20 relative overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="marquee-container py-10">
          <div className="marquee-content">
            {sponsorLogos.map((sponsor, index) => (
              <Card key={index} className="sponsor-card magnetic-button glass-card border-2 hover:border-hackathon-pink hover-float">
                <CardContent className="flex flex-col items-center justify-center p-8 h-40 w-64">
                  <div className={cn("w-16 h-16 rounded-full flex items-center justify-center mb-4", sponsor.color)}>
                    <span className="text-2xl font-bold text-white">{sponsor.name.charAt(0)}</span>
                  </div>
        ...
        <div className="marquee-container py-10 mt-6">
          <div className="marquee-content" style={{ animationDirection: 'reverse' }}>
            {sponsorLogos.slice().reverse().map((sponsor, index) => (
              <Card key={index} className="sponsor-card magnetic-button glass-card border-2 hover:border-hackathon-cyan hover-float">
                <CardContent className="flex flex-col items-center justify-center p-8 h-40 w-64">
                  <div className={cn("w-16 h-16 rounded-full flex items-center justify-center mb-4", sponsor.color)}>
                    <span className="text-2xl font-bold text-white">{sponsor.name.charAt(0)}</span>
                  </div>
